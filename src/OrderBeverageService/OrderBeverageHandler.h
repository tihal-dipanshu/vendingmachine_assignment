...
void OrderBeverageServiceHandler::PlaceOrder(std::string& _return, const int64_t city) {
    ...
    try {
      ...
      LOG(error) << "Failed to get weather from weather-service";
      throw;
    }
    _weather_client_pool->Push(weather_client_wrapper);
    ...
    BeverageType::type btype = (weather == WeatherType::WARM) ? 
        BeverageType::COLD : BeverageType::HOT;
    
    // Get beverage from beverage preference service
    ...
    try {
      _return = beverage_client->getBeverage(btype);
    } catch (...) {
      _beverage_client_pool->Push(beverage_client_wrapper);
      LOG(error) << "Failed to get beverage from beverage-preference-service";
      throw;
    }
    _beverage_client_pool->Push(beverage_client_wrapper);
}
